...
def register_alarm_family_admin_callback() -> None:
    register_admin_callback(
        definition_factory=lambda: ALARM_FAMILY_ADMIN_DEFINITION,
...
def _build_data_service() -> AlarmGuardedAdminDataService:
    ...
    return AlarmGuardedAdminDataService(
        delegate=delegate,
        normalize_rows=AlarmFamilyRowNormalizationService.normalize_rows,
        validate_rows=lambda previous_rows, next_rows: guard_service.validate_family_rows(
