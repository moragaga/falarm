...
def register_alarm_tools_admin_callback() -> None:
    register_admin_callback(
        definition_factory=lambda: ALARM_TOOLS_ADMIN_DEFINITION,
...
def _build_data_service() -> AlarmGuardedAdminDataService:
    ...
    return AlarmGuardedAdminDataService(
        delegate=delegate,
        validate_rows=lambda previous_rows, next_rows: guard_service.validate_tool_rows(
