...
class AlarmSubcomponentRowFactoryService:
    @staticmethod
    def build_new_row(
        *,
        ...
        return {
            'subcomponent_key': f'alarm_subcomponent_{uuid4().hex[:8]}',
            'subcomponent_code': '',
            'subcomponent_name': '',
            'parent_component_key': '',
            'display_order': AlarmSubcomponentRowFactoryService._resolve_next_order(rows=rows),
            'is_active': True,
        }
