...
def register_alarm_components_admin_callback() -> None:
    register_admin_callback(
        definition_factory=lambda: ALARM_COMPONENTS_ADMIN_DEFINITION,
...
def _build_data_service() -> AlarmGuardedAdminDataService:
    ...
    return AlarmGuardedAdminDataService(
        delegate=delegate,
        validate_rows=lambda previous_rows, next_rows: guard_service.validate_component_rows(
